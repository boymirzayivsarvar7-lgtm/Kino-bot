...
async def save_movie(cb: CallbackQuery, state: FSMContext):
    ...
    try:
        data = await state.get_data()
        await db_exec(
            "INSERT INTO movies (code,title,year,country,genre,director,duration,file_id_480,file_id_1080) VALUES (?,?,?,?,?,?,?,?,?)",
            (data['kod'], data['nom'], data.get('yil',''), data.get('mamlakat',''), data.get('janr',''), data.get('rejissyor',''), data.get('davomiylik',''), data.get('file_id_480',''), data.get('file_id_1080') or '')
        )
        await state.clear()
        await cb.message.edit_text(
...
async def confirm_add_movie_cb(cb: CallbackQuery, state: FSMContext):
    if cb.from_user.id != ADMIN_ID:
        return
    try:
        data = await state.get_data()
        await db_exec(
            "INSERT INTO movies (code,title,year,country,genre,director,duration,file_id_480,file_id_1080) VALUES (?,?,?,?,?,?,?,?,?)",
            (data['kod'], data['nom'], data.get('yil',''), data.get('mamlakat',''),
             data.get('janr',''), data.get('rejissyor',''), data.get('davomiylik',''),
             data.get('file_id_480',''), data.get('file_id_1080') or '')
        )
        await state.clear()
        await cb.message.edit_text(
        ...
        log.error(f"Confirm add movie xatosi: {e}")
        await cb.answer("❌ Xatolik!")

@router.callback_query(F.data == "cancel_add_movie")
async def cancel_add_movie_cb(cb: CallbackQuery, state: FSMContext):
    if cb.from_user.id != ADMIN_ID:
        return
    await state.clear()
    await cb.message.edit_text("❌ Kino qo'shish bekor qilindi.", reply_markup=back_kb())
    await cb.answer()
