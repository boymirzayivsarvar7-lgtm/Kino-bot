"""
🎬 KINO BOT - Polling rejimi (Render.com uchun)
"""
...
async def add_movie_kod(msg: Message, state: FSMContext):
    ...
    if await get_movie(code):
        await msg.answer(f"⚠️ <b>{code}</b> kodi allaqachon mavjud!")
        return
...
async def add_movie_nom(msg: Message, state: FSMContext):
    ...
    await msg.answer("3️⃣ Kino <b>yilini</b> yuboring:\n/skip — o'tkazib yuborish")

@router.message(AddMovie.yil)
async def add_movie_yil(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    await state.update_data(yil="" if msg.text.strip() == "/skip" else msg.text.strip())
...
async def add_movie_mamlakat(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    await state.update_data(mamlakat="" if msg.text.strip() == "/skip" else msg.text.strip())
...
async def add_movie_janr(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    await state.update_data(janr="" if msg.text.strip() == "/skip" else msg.text.strip())
...
async def add_movie_rejissyor(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    await state.update_data(rejissyor="" if msg.text.strip() == "/skip" else msg.text.strip())
...
async def add_movie_480(msg: Message, state: FSMContext):
    ...
    try:
        ...
        davomiylik = f"{dur // 60}:{dur % 60:02d}"
        ...
        await state.set_state(AddMovie.video_1080)
        await msg.answer(
            f"✅ 480p saqlandi! ⏱ {davomiylik}\n\n"
            f"8️⃣ <b>1080p</b> videoni yuboring:\n/skip — o'tkazib yuborish"
        )
    except Exception as e:
        log.error(f"Video 480 xatosi: {e}")
        await msg.answer("❌ Xatolik. Bot kanalda admin ekanligini tekshiring.")
...
async def add_movie_1080_skip(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    await state.update_data(file_id_1080="")
    await save_movie_to_db(msg, state)
...
async def save_movie_to_db(msg: Message, state: FSMContext):
    try:
        data = await state.get_data()
        await db_exec(
            "INSERT INTO movies (code,title,year,country,genre,director,duration,file_id_480,file_id_1080) "
            "VALUES (?,?,?,?,?,?,?,?,?)",
            (
                data['kod'], data['nom'],
                data.get('yil', ''), data.get('mamlakat', ''),
                data.get('janr', ''), data.get('rejissyor', ''),
                data.get('davomiylik', ''),
        ...
        log.error(f"Kino saqlash xatosi: {e}")
        await msg.answer("❌ Xatolik. Kod allaqachon mavjud bo'lishi mumkin.")
...
async def del_movie_kod(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="🗑 Ha", callback_data="confirm_del"),
...
async def confirm_del(cb: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await cb.message.edit_text(f"✅ <b>{data['nom']}</b> o'chirildi.", reply_markup=back_kb())
...
async def adm_list_movies(cb: CallbackQuery):
    ...
    for m in movies:
        lines.append(f"• <code>{m['code']}</code> — {m['title']} ({m.get('year') or '—'})")
    text = "\n".join(lines)
    if len(text) > 4000:
        text = text[:4000] + "\n..."
    await cb.message.edit_text(text, reply_markup=back_kb())
...
async def adm_vip_menu(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="⚠️ 3 kunda tugaydiganlar", callback_data="vip_expiring")],
            [InlineKeyboardButton(text="➕ VIP berish", callback_data="vip_manual_add")],
...
async def vip_expiring(cb: CallbackQuery):
    ...
    if not users:
        await cb.message.edit_text("✅ 3 kun ichida tugaydigan VIP yo'q.", reply_markup=back_kb("adm_vip"))
        await cb.answer()
        return
    lines = [f"⚠️ <b>3 kunda tugaydiganlar</b> ({len(users)} ta)\n"]
    for u in users:
...
async def vip_manual_add_start(cb: CallbackQuery, state: FSMContext):
    if cb.from_user.id != ADMIN_ID:
        return
    await state.set_state(ManualVip.uid)
...
@router.message(ManualVip.uid)
async def manual_vip_uid(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    try:
        ...
        if data["action"] == "remove":
            ...
            await state.clear()
            await msg.answer(f"✅ <code>{uid}</code> dan VIP olindi.")
            try:
                await bot.send_message(uid, "ℹ️ VIP a'zoligingiz admin tomonidan bekor qilindi.")
            except Exception:
                pass
        else:
            await state.update_data(uid_target=uid)
...
async def manual_vip_kunlar(cb: CallbackQuery, state: FSMContext):
    ...
    try:
        await bot.send_message(uid, f"🎉 Sizga {kunlar} kunlik VIP berildi!\n📅 ({exp_str} gacha)")
    except Exception:
        pass
...
async def receive_check(msg: Message, state: FSMContext):
    try:
        ...
        await bot.send_photo(ADMIN_ID, photo=photo_id, caption=caption, reply_markup=confirm_vip_kb(req_id, user.id))
    except Exception as e:
...
async def broadcast_target(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(target=cb.data.replace("bc_", ""))
    await cb.message.edit_text("✍️ Xabarni yuboring (matn, rasm yoki video):", reply_markup=back_kb("adm_broadcast"))
    await cb.answer()
...
async def do_broadcast(msg: Message, state: FSMContext):
    ...
    for u in users:
        try:
            ...
            failed += 1
    await msg.answer(f"📢 <b>Broadcast yakunlandi!</b>\n\n✅ Yuborildi: {sent}\n❌ Xato: {failed}", reply_markup=admin_inline_kb())
...
async def adm_settings(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="💰 VIP narxlari", callback_data="set_vip_price")],
...
async def set_karta_save(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer(f"✅ Karta yangilandi:\n<code>{msg.text.strip()}</code>")
...
async def add_channel_save(msg: Message, state: FSMContext):
    ...
    text = msg.text.strip()
    try:
        chat_id = int(text) if text.lstrip("-").isdigit() else (text if text.startswith("@") else f"@{text}")
...
async def send_movie_to_user(user_id: int, movie_code: str):
    try:
        ...
        if vip and movie.get("file_id_1080"):
            file_id = movie["file_id_1080"]
            quality = "1080p"
            protect = False
            kb = movie_vip_kb(movie_code)
        else:
            file_id = movie.get("file_id_480", "")
            quality = "480p"
            protect = True
        ...
        await bot.send_video(user_id, video=file_id, caption=caption, protect_content=protect, reply_markup=kb)
...
async def rate_menu(cb: CallbackQuery):
    ...
    if not movie:
        ...
        return
    await cb.message.answer(f"⭐ <b>{movie['title']}</b> filmini baholang:", reply_markup=rating_kb(movie_code))
    await cb.answer()
...
async def movie_code_handler(msg: Message, state: FSMContext):
    try:
        ...
        bot_active = await get_setting("bot_active")
        if bot_active == "0" and msg.from_user.id != ADMIN_ID:
            await msg.answer("🔧 Bot hozirda to'xtatilgan.")
            return
        await register_user(msg.from_user)
        code = msg.text.strip().upper()
        movie = await get_movie(code)
...
async def check_vip_expiry():
    try:
        ...
        expired = await db_all("SELECT user_id FROM users WHERE vip_status=1 AND vip_expiry<?", (now,))
        for u in expired:
            await db_exec("UPDATE users SET vip_status=0 WHERE user_id=?", (u["user_id"],))
            try:
                await bot.send_message(u["user_id"], "⏰ <b>VIP muddatingiz tugadi!</b>\n\nYangi VIP olish uchun 🌟 VIP tugmasini bosing.")
            except Exception:
                pass
        if expired:
            log.info(f"⏰ {len(expired)} ta VIP muddati tugadi")
    except Exception as e:
